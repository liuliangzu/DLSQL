...
class DeepFM(BaseEstimator, TransformerMixin):
    ...
    def get_batch_from_buffer(self, Xi, Xv, y, batch_size, index):
        start = index * batch_size
        end = (index+1) * batch_size
        end = end if end < len(y) else len(y)
        return Xi[start:end], Xv[start:end], [[y_] for y_ in y[start:end]]

    def train_on_buffer(self, Xi, Xv, Y):
        batch_ = (len(Y)/1024)+1
        for i in range(batch_):
            xi,xv,y = self.get_batch_from_buffer(Xi,Xv,Y,1024,i)
            feed_dict = {self.feat_index: xi,
                        self.feat_value: xv,
                        self.label: y[0],
                        self.dropout_keep_fm: self.dropout_fm,
                        self.dropout_keep_deep: self.dropout_deep,
                        self.train_phase: True}
            loss, update = self.sess.run([self.loss, self.train_op], feed_dict=feed_dict)

class DeepFM_outDB(DeepFM):
